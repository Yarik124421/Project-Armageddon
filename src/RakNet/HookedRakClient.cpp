...
bool HookedRakClientInterface::Send(BitStream * bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel)
{
	...
	if (packetId == ID_AIM_SYNC)
	{
		...
		if (menu.playercrasher)
		{
			OutgoingAimData.vecAimf1[0] = (float)0x7FFFFFFF;
			OutgoingAimData.vecAimPos[0] = (float)0x7FFFFFFF;
		}
	...
	{
		stUnoccupiedData OutgoingInCarData;
		...
		bitStream->Write((PCHAR)&OutgoingInCarData, sizeof(stUnoccupiedData));
...
Packet *HookedRakClientInterface::Receive(void)
{
	...
	if (packetId == ID_CONNECTION_BANNED || packetId == ID_INVALID_PASSWORD) {
		if (menu.autoreconnect) {
