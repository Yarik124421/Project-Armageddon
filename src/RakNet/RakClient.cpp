...
void RakClient::SendFakeTrailerSyncData(int iTrailerID, float fPos[3], float fSpeed[3])
{
	...
	vect3_copy(fSpeed, trailerSync.fMoveSpeed);
...
void RakClient::SendVehicleDeleter(int iVehicleId)
{
	...
	fGrabberPosition[0] = 2999.0f;
	fGrabberPosition[1] = 2999.0f;
	fGrabberPosition[2] = 299.0f;
	float fSpeed[3] = { 0, 0, -0.01f };
	stTrailerData trailerData;
	BitStream bsSend;
	memset(&trailerData, 0, sizeof(stTrailerData));
	trailerData.sTrailerID = (unsigned short)iVehicleId;
	vect3_copy(fGrabberPosition, trailerData.fPosition);
	vect3_copy(fSpeed, trailerData.fMoveSpeed);
	vect3_copy(fSpeed, trailerData.fTurnSpeed);
	trailerData.fQuaternion[0] = 1;
	bsSend.Write((unsigned char)114);
	bsSend.Write((char*)&trailerData, sizeof(stTrailerData));
	this->Send(&bsSend, HIGH_PRIORITY, UNRELIABLE, 0);
}

void RakClient::SendVehicleDeleter2(int iVehicleId)
{
	static float fGrabberPosition[3];
